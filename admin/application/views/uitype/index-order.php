...
<form method="post" enctype="multipart/form-data">
	<div class="text-pt">
		<div class="btn-sub">
			...
			<?php  } ?>
			<button type="submit" name="insert" onclick=myFunction(); class="btn btn-primary">Save</button>
		</div>
	</div>
	<?php 	foreach ($allArray['blocks'] as $b => $blocks) { ?>
			<div class="<?=$blocks->blocksid ?>">
							...
							<?php 	if ($value->block == $blocks->blocksid) { ?>
								...
								<?php 	}else { ?>
									<?php 	if ($value->uitype =='1') { ?>
										...
										<?php if ($value->columnname=='transactionid' ) {?>
										...
										<?php }  else {?>
											<input type="text" <?=$readonly;?> value="<?php echo $inputFieldValues[$value->columnname]; ?>" name="<?php echo $value->columnname?>" id="<?php echo $value->columnname?>" class="form-control" placeholder="<?php echo $value->fieldlabel?>">
										<?php } } else { ?>
										<input type="text" <?=$readonly;?> value="<?php echo $inputFieldValues[$value->columnname]; ?>" name="<?php echo $value->columnname?>" id="<?php echo $value->columnname?>" class="form-control" placeholder="<?php echo $value->fieldlabel?>">
										<?php } ?>
										</div>
									...
									<?php 	if ($value->uitype =='2') { ?>
											...
											<textarea type="text" <?=$readonly;?>  name="<?php echo $value->columnname?>" class="form-control" placeholder="<?php echo $value->fieldlabel?>"><?php echo $inputFieldValues[$value->columnname]; ?></textarea>
										</div>
					                <?php  	} ?>
										...
										<?php 	foreach ($allArray['dropdown'] as $drop => $down) { ?>
											...
											<?php 	if ($d.'id' == $value->columnname && !empty($allArray[$down->fieldname])) { ?>
												<?php  $typeofdata = explode('~', $value->typeofdata); if(strtoupper($typeofdata[1]) == 'M') $required = 'required';else $required = ''; ?>
																...
																<?php 	} else if($value->columnname == 'orderstatusid'){ ?>
																	<?php 	$onchange = ' onchange="getorderStatusId(this.value,\'returnreason\',\'cancelreason\',\'returndate\',\'canceldate\')"';?>
																	<?php 	$orderstatusid = $inputFieldValues[$value->columnname]; ?>
																<?php 	}else{	?>
																	<?php 	$onchange = '';?>
																<?php 	}	?>
									...
									<?php 	if ($value->uitype =='4') { ?>
										<div class="input-group txt-frm mb-3 <?php echo $value->columnname?>Div">
											...
											<?php 	if ($this->session->userdata('role_id') == 1) { ?>
										<?php if ($value->columnname=='paymentreceiveddate' ) {?>
												<?php 	if (!empty(strtotime($inputFieldValues[$value->columnname])) && strtotime($inputFieldValues[$value->columnname]) > 0) { ?>
													<input required type="date" <?=$readonly;?> value="<?php echo $inputFieldValues[$value->columnname]; ?>" name="<?php echo $value->columnname?>" id="<?php echo $value->columnname?>" class="form-control" placeholder="<?php echo $value->fieldlabel?>">
												<?php 	}else{ ?>
													<input required type="date" value="<?php echo $inputFieldValues[$value->columnname]; ?>" id="<?php echo $value->columnname?>" name="<?php echo $value->columnname?>" class="form-control" placeholder="<?php echo $value->fieldlabel?>">
												<?php 	} ?>
										<?php 	} else { ?>
										<input type="date" <?=$readonly;?> value="<?php echo $inputFieldValues[$value->columnname]; ?>" id="<?php echo $value->columnname?>" name="<?php echo $value->columnname?>" class="form-control" placeholder="<?php echo $value->fieldlabel?>">
										<?php } } else{ ?>
												<input type="date" <?=$readonly;?> value="<?php echo $inputFieldValues[$value->columnname]; ?>" id="<?php echo $value->columnname?>" name="<?php echo $value->columnname?>" class="form-control" placeholder="<?php echo $value->fieldlabel?>">
											<?php 	} ?>
										</div>
											...
											<div class="input-group-prepend">
												<select class="input-group-text" name="<?php echo $value->columnname?>withcode" id="<?php echo $value->columnname?>withcode">
													<?php if($this->uri->segment(2) == 'edit') { ?>
														...
														<?php foreach ($allArray['countryphonecode'] as $ki => $vi) { if('+'.$vi['phonecode'] == $allArray['records'][$value->columnname.'withcode']) $selected = 'selected';else $selected = '';?>
...
<script>
var tqa='<?php echo $this->session->userdata["role_id"] ?>';
var mdid='<?php echo $allArray['module']->moduleid ?>';
var roleid='<?php echo $this->session->userdata["role_id"] ?>';
var orderstatusid=$('#orderstatusid').val();
if(tqa=='3' && mdid=='5'){
if(orderstatusid=='2'){
    $('.purchaseddateDiv').css('display','none');
    $('.ordermodeidDiv').css('display','none');
    $('.shipchargeDiv').css('display','none');
    $('.currencycodeDiv').css('display','none');
    $('.awbnumberDiv').css('display','none');
    $('.delivery_dateDiv').css('display','none');
    $('.othertaxDiv').css('display','none');
    $('.delivery_dateDiv').css('display','none');
    $('.paymentreceiveddateDiv').css('display','none');   
    $('.transactionidDiv').css('display','none'); 
    $('.paymentmodeidDiv').css('display','none');   
    $('.deliverystatusDiv').css('display','none');   
    $('.paymentmodeidDiv').css('display','none'); 
...
{
    if(roleid =='1'){
       var r= confirm("Are you sure want to continue with the changes? Have you filled Payment received date, Transaction Id and Order Status!!");
        if (r==true)
          {
          x="You pressed OK!";
          }
    ...
    if(roleid =='3'){
       if(orderstatusid=='2'){
       var r= confirm("Are you sure want to continue with the changes? Have you filled Reason of Return and Return Date / reason of Cancel and Cancel Date!!");
       }else{
        var r= confirm("Are you sure want to continue with the changes? Have you filled Reason of Cancel and Cancel Date!!");
...
function getorderStatusId(id,field1,field2,field3,field4){
	//console.log(id)
	//console.log(field)
	if(id == 3){
		$("#"+field1).attr('required','required');
		$("#"+field3).attr('required','required');
		$("#"+field2).removeAttr('required');
		$("#"+field4).removeAttr('required');
	}else if(id == 4){
		$("#"+field2).attr('required','required');
		$("#"+field4).attr('required','required');
