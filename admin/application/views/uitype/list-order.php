		...
		<h6>
			...
			<!--<span>                           
				<div class="form-group left-prt exprt" >
 					<a href="<?=base_url($this->uri->segment(1).'/'.$this->uri->segment(2).'/phpexcel_export/')?>"><button type="button" class="btn btn-success">Export Excel</button></a>
 				</div>
 			</span>-->
 			<?php } ?>
			<!--<span>                           
							...
							<tr>
								...
								<td><?php 	foreach ($allArray['modulepermission'] as $keymp => $valuemp) { ?>
											...
											<?php 	}else if($valuemp['modulemodeid'] == 3){ ?>
												<?php 	if(!$valuemp['presence']) { ?>
										<?php if($this->session->userdata["role_id"] =='1' && $allArray['ordermode']=='orderreturn'){?>
										<?php } else if($this->session->userdata["role_id"] =='1' && $allArray['ordermode']=='ordercancel'){?>
										<?php } else if($this->session->userdata["role_id"] =='1' && $allArray['ordermode']=='orderreceiveds'){?>
										<?php } else if($this->session->userdata["role_id"] =='3' && $allArray['ordermode']=='orderpending'){?>
										<?php } else if($this->session->userdata["role_id"] =='3' && $allArray['ordermode']=='ordercancel'){?>
										<?php } else if($this->session->userdata["role_id"] =='3' && $allArray['ordermode']=='orderreturn'){?>
										<?php } else { ?>
