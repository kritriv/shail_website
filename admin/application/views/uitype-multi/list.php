<div class="banner1">    
	...
	<div class="log right-prt">
		...
		<?php if($this->uri->segment(1) == 'profile' || $this->uri->segment(1) == 'wishlist' || $this->uri->segment(1) == 'review' || $this->uri->segment(1) == 'productbid' || $this->uri->segment(1) == 'sliders' || $this->uri->segment(1) == 'uimenu' || $this->uri->segment(1) == 'website' || $this->uri->segment(1) == 'type' || $this->uri->segment(1) == 'saletype' || $this->uri->segment(1) == 'newsletter' || $this->uri->segment(1) == 'comments'){  ?>
		...
		<?php }else{ ?>
			...
			<?php 	foreach ($allArray['modulepermission'] as $keymp => $valuemp) { ?>
				...
				<?php if($valuemp['modulemodeid'] == 1) { ?>
                    <?php if(($roleid=='5' || $roleid=='3') && ($moduleid=='page' || $moduleid=='postcategory' || $moduleid=='social')){ ?>
                    <?php } else { ?>
				...
				<?php if($this->uri->segment(1) == 'product'){ ?>
					...
					<div class="form-group">
						...
						<select name="soldqty" id="soldqtyfilters" class="form-control border-radius-20">
							...
							<option value="0">Select Never Purchaged</option>
							...
							<?php $i=0;	foreach ($allArray['records'] as $key => $value) { $i++;?>
								...
								<tr>
									...
									<td>
										...
										<select onchange="getValueAndGo(this.value)" name="languageid">

											<option><img src="<?=base_url('asset/image/Eye_37136.png')?>" alt="" class="im-2">View</option>

											<?php  foreach ($allArray['modulepermission'] as $keymp => $valuemp) { ?>

												<?php 	foreach ($allArray['alllanguage'] as $p => $q) { ?>

													<?php 	if($valuemp['modulemodeid'] == 2) { ?>

														<?php 	if(!$valuemp['presence']) { ?>

															<option value="<?=base_url($segment.'/view/'.$value[$segment.'id'])?>?languageid=<?=$q['languageid']?>"><?=$q['languagecode'];?>
										...
										<select onchange="getValueAndGo(this.value)">

											<option><img src="<?=base_url('asset/image/Actions-document-edit-icon.png')?>" class="edit-img">Edit</option>

											<?php  foreach ($allArray['modulepermission'] as $keymp => $valuemp) { ?>

												<?php 	foreach ($allArray['alllanguage'] as $p => $q) { ?>

													<?php 	if($valuemp['modulemodeid'] == 3){ ?>

														<?php 	if(!$valuemp['presence']) { ?>

															<option value="<?=base_url($segment.'/edit/'.$value[$segment.'id'])?>?languageid=<?=$q['languageid']?>">

															<a href="<?=base_url($segment.'/edit/'.$value[$segment.'id'])?>?languageid=<?=$q['languageid']?>" title="<?=$value['mode']?>"><img src="<?=base_url('asset/image/Actions-document-edit-icon.png')?>" class="edit-img"></a><?=$q['languagecode'];?>
