...
class Social extends MY_Controller {
	function __construct() {
        parent::__construct();
        $this->load->model('social_model', '', TRUE);
    ...
    private function initializeInputFieldValues($id = 0) {
        ...
        if ($id == 0) {
            ...
            $row = $this->social_model->getRowByColumnArray($this->uri->segment(1).'id',$id);
            foreach ($this->viewData['modulefield'] as $key => $value) {
    ...
    public function deleteimage() {
        echo $this->social_model->deletefile($this->uri->segment(1).'id',$_POST['id'],$_POST['columnname'],$_POST['filename'],$_POST['t']);
    }
    public function phpexcel_export(){
        //if(isset($_POST['export'])){
            $f = $this->viewData['module']; 
            $id = $f->moduleid;
            $columnlistArray = $this->social_model->getAllColumnArray($id);
            $export_data = $this->social_model->getAllAvailableArray();
            $this->excelExport($columnlistArray,$export_data);
        //}
    }
	public function index() {
        $limit_per_page = LIMIT_PER_PAGE;
        $total_records = $this->social_model->get_total();
        ...
        $f = $this->viewData['module']; 
        $id = $f->moduleid;
        $this->viewData['columnlist'] = $this->social_model->getAllColumnArray($id);
        $this->viewData['records'] = $this->social_model->getAllAvailableArray($limit_per_page,$start_index);
    ...
    public function view($id) {
        ...
        $this->viewData['records'] = $this->social_model->getRowByColumn($this->uri->segment(1).'id',$id);
        $d['allArray'] = $this->viewData;
    ...
    public function edit($id) {
        ...
        if (isset($_POST['insert'])) {
            ...
            unset($post['insert']);
            $inserted = $this->social_model->update($id,$post);
            if($inserted){
        ...
        $this->viewData['records'] = $this->social_model->getRowByColumn('socialid',$id);
        $d['allArray'] = $this->viewData;
        $d['inputFieldValues'] = $inputFieldValues;
    ...
    public function delete($id) {
        ...
        $this->social_model->delete($this->uri->segment(1).'id',$id);
        $this->session->set_flashdata('flash_success_msg', 'The Social Icon has been deleted successfully.');
        redirect($this->uri->segment(1)."/index");
    }
    public function deleteAll() {
        $this->deletePermission();
        $this->social_model->delete($this->uri->segment(1).'id',$this->input->post('ckbdelete'));
        $this->session->set_flashdata('flash_success_msg', 'The Social Icon has been deleted successfully.');
        redirect($this->uri->segment(1)."/index");
    }
    public function download(){
        $csvResultOfProjects = $this->social_model->getAllAvailableArrayDownload();
    ...
    public function add() {
        ...
        if (isset($_POST['insert'])) {
            ...
            unset($post['insert']);
            $inserted = $this->social_model->insert($post);
            $moduleid = $this->viewData['module']->moduleid;
            if($inserted){
                $this->session->set_flashdata('flash_success_msg', 'The New Social Icon has been added successfully.');
                redirect($this->uri->segment(1)."/index");
            }else{
                $this->session->set_flashdata('flash_error_msg', 'Some Problem occured.');
            }
