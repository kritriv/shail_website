...
class banner extends MY_Controller {
    ...
    private function initializeInputFieldValues($id = 0) {
        ...
        if ($id == 0) {
            ...
            $row = $this->banner_model->getRowByColumnArray($this->uri->segment(1).'id',$id);
            foreach ($this->viewData['modulefield'] as $key => $value) {
	...
	public function index() {
        ...
        $total_records = $this->banner_model->get_total();
        ...
        $id = $f->moduleid;
        $this->viewData['columnlist'] = $this->banner_model->getAllColumnArray($id);
        $this->viewData['records'] = $this->banner_model->getAllAvailableArray($limit_per_page,$start_index);
    ...
    public function view($id) {
        ...
        $this->viewData['records'] = $this->banner_model->getRowByColumn($this->uri->segment(1).'id',$id);
        $d['allArray'] = $this->viewData;
    ...
    public function edit($id) {
        ...
        if (isset($_POST['insert'])) {
            ...
            unset($post['insert']);
            $inserted = $this->banner_model->update($id,$post);
            if($inserted){
    ...
    public function delete($id) {
        ...
        $this->banner_model->delete($this->uri->segment(1).'id',$id);
        $this->session->set_flashdata('flash_success_msg', 'The Product Type has been deleted successfully.');
        redirect($this->uri->segment(1)."/index");
    }
    public function deleteAll() {
        $this->deletePermission();
        $this->banner_model->delete($this->uri->segment(1).'id',$this->input->post('ckbdelete'));
        $this->session->set_flashdata('flash_success_msg', 'The Product Type has been deleted successfully.');
        redirect($this->uri->segment(1)."/index");
    }
    public function deleteimage() {
        echo $this->banner_model->deletefile($this->uri->segment(1).'id',$_POST['id'],$_POST['columnname'],$_POST['filename'],$_POST['t']);
    }
    public function download(){
        $csvResultOfProjects = $this->banner_model->getAllAvailableArrayDownload();
    ...
    public function add() {
        ...
        if (isset($_POST['insert'])) {
            ...
            unset($post['insert']);
            $inserted = $this->banner_model->insert($post);
            $moduleid = $this->viewData['module']->moduleid;
            if($inserted){
                $this->session->set_flashdata('flash_success_msg', 'The New Product Type has been added successfully.');
                redirect($this->uri->segment(1)."/index");
            }else{
                $this->session->set_flashdata('flash_error_msg', 'Some Problem occured.');
            }
