...
class Media extends MY_Controller {
    ...
    private function initializeInputFieldValues($id = 0) {
        ...
        if ($id == 0) {
            ...
            $row = $this->media_model->getRowByColumnArray($this->uri->segment(1).'id',$id);
            foreach ($this->viewData['modulefield'] as $key => $value) {
    ...
    public function phpexcel_export(){
        //if(isset($_POST['export'])){
            ...
            $id = $f->moduleid;
            $columnlistArray = $this->media_model->getAllColumnArray($id,'sequence');
            $export_data = $this->media_model->getAllAvailableArray();
	...
	public function index() {
        ...
        $f = $this->viewData['module']; 
        $id = $f->moduleid;
        $this->viewData['columnlist'] = $this->media_model->getAllColumnArray($id);
        $this->viewData['records'] = $this->media_model->getAllAvailableArray($limit_per_page,$start_index);
    ...
    public function view($id) {
        ...
        $this->viewData['records'] = $this->media_model->getRowByColumn($this->uri->segment(1).'id',$id);
        $d['allArray'] = $this->viewData;
        $d['inputFieldValues'] = $inputFieldValues;
        $this->load->view($this->uri->segment(1)."/view",$d);
    }
    public function deleteimage() {
        echo $this->media_model->deletefile($this->uri->segment(1).'id',$_POST['id'],$_POST['columnname'],$_POST['filename'],$_POST['t']);
    }
    public function edit($id) {
        ...
        if (isset($_POST['insert'])) {
            ...
            unset($post['insert']);
            $inserted = $this->media_model->update($id,$post);
            if($inserted){
        ...
        $this->viewData['records'] = $this->media_model->getRowByColumn('Mediaid',$id);
        $d['allArray'] = $this->viewData;
        $d['inputFieldValues'] = $inputFieldValues;
    ...
    public function delete($id) {
        ...
        $this->media_model->delete($this->uri->segment(1).'id',$id);
        $this->session->set_flashdata('flash_success_msg', 'The Media has been deleted successfully.');
        redirect($this->uri->segment(1)."/index");
    }
    public function deleteAll() {
        $this->deletePermission();
        $this->media_model->delete($this->uri->segment(1).'id',$this->input->post('ckbdelete'));
        $this->session->set_flashdata('flash_success_msg', 'The Media has been deleted successfully.');
        redirect($this->uri->segment(1)."/index");
    }
    public function download(){
        $csvResultOfProjects = $this->media_model->getAllAvailableArrayDownload();
    ...
    public function add() {
        ...
        if (isset($_POST['insert'])) {
            ...
            unset($post['insert']);
            $inserted = $this->media_model->insert($post);
            $moduleid = $this->viewData['module']->moduleid;
            if($inserted){
                $this->session->set_flashdata('flash_success_msg', 'The New Media has been added successfully.');
                redirect($this->uri->segment(1)."/index");
            }else{
                $this->session->set_flashdata('flash_error_msg', 'Some Problem occured.');
            }
